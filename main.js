...
var processInfo = {}; //Dictionary containing info about each process. Keys will be id and neighbor
			...
			Structure of dictionary:
				{
					'message': 'Initiate',
					'id': <id of process>,
					'neighbor': <dictionary containing two keys: neighborID and edge weight, members of dict are all neighbors of process>				}
		*/
	...
	switch(lineNum) {
		...
		default:
			...
			line.split(' ').forEach((wt, idx) => {
				if(wt!=='-1.0') {
					processInfo[process][processArrConst[idx]] = wt;
				}
...
function readInput() {
	...
	const reader =  readline.createInterface({
		input : require('fs').createReadStream('connectivity.txt')
	});
	lineNum = 1;
...
async function main() {
	...
	*/
	...
	/*
		Instead of getting acks from clients (thats its done with the current step), sending process to sleep.
